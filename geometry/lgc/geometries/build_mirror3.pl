sub make_mirror3
{
    my $V0_tg_V = vector(0.,0.,$ztarg_cent3);

    $R = 0.;  #zero out buildSPmirror globals
    $PosV = vector(0.,0.,0.);
    
    my $mirrRotPoint = vector(0.0, (-$V0_tg_V->z() + $zMirrRotPoint3)*tan($cr_ang3*$D2R), $zMirrRotPoint3);

    my $Z_front = 200.0 - $V0_tg_V->z();
    my $Z_end = 301.0 - $V0_tg_V->z();
    my $R_front_in = $Z_front*tan($Angle_in3*$D2R);
    my $R_front_out = $Z_front*tan($Angle_out3*$D2R);
    my $R_end_in = $Z_end*tan($Angle_in3*$D2R);
    my $R_end_out = $Z_end*tan($Angle_out3*$D2R);

    &buildSPmirror($V0_tg_V, $Pos_im_Obs_V, $cr_ang3, $Z_M3);
    my $PosV_temp = $PosV;
    my $PosV_cone = vector(0.,0.,250.5);
    my $PosV_delta = vector(0., 0., 0.);
    my $PosV_deltaC = vector(0., 0., 0.);

    print "MIRROR 3:\n";
    print "sphere center $PosV\n";
    print "sphere R: $R\n";
  &calcIntPoints($R, $PosV, $PosV_cone, 0.5*($Z_end - $Z_front), $R_front_in, $R_front_out, $R_end_in, $R_end_out);

    
    if($mirrAng3){
	$PosV_delta = mirrorRot($cr_ang3, $PosV_temp, $mirrAng3, $ztarg_cent3, $zMirrRotPoint3);
	$PosV_temp = $PosV_temp + $PosV_delta;
    

	$PosV_deltaC = mirrorRot($cr_ang3, $PosV_cone, $mirrAng3, $ztarg_cent3, $zMirrRotPoint3);

	$PosV_cone = $PosV_cone + $PosV_deltaC;
    }
    my $PosV_temp_CM = $PosV_temp;
        

    for(my $n=1; $n<=$Nsec; $n++)

    {
	$PosV_temp = &rotateZv($PosV_temp,12.0*$D2R);

	$detector{"name"}        = $namePre."Mirror3s_$n";
	$detector{"mother"}      = $namePre."Tank" ;
	$detector{"description"} = "Mirror3 Sphere number $n";
	$detector{"pos"}        = sprintf('%.3f',$PosV_temp->x())."*cm ".sprintf('%.3f',$PosV_temp->y())."*cm ".sprintf('%.3f',$PosV_temp->z())."*cm";
	$detector{"rotation"}   = "0*deg 0*deg 0*deg";
	$detector{"color"}      = "66bbff";
	$detector{"type"}       = "Sphere";
	$detector{"dimensions"} = sprintf('%.3f',$R)."*cm ". sprintf('%.3f',$R + 0.1*$T_M3)."*cm 0.0 360.0*deg 0.0 90.0*deg";
	$detector{"material"}   = "Component";
	$detector{"mfield"}     = "no";
	$detector{"ncopy"}      = 1;
	$detector{"pMany"}       = 1;
	$detector{"exist"}       = 1;
	$detector{"visible"}     = 1;
	$detector{"style"}       = 1;
	$detector{"sensitivity"} = "no";
	$detector{"hit_type"}    = "";
	$detector{"identifiers"} = "Mirror3 sphere $n";
	
	print_det(\%configuration, \%detector);

	$PosV_cone = &rotateZv($PosV_cone,12.0*$D2R);

	$detector{"name"}        = $namePre."Mirror3c_$n";
	$detector{"mother"}      = $namePre."Tank" ;
	$detector{"description"} = "Mirror3 cone $n";
	$detector{"pos"}        = sprintf('%.3f',$PosV_cone->x())."*cm ".sprintf('%.3f',$PosV_cone->y())."*cm ".sprintf('%.3f',$PosV_cone->z())."*cm";
	$detector{"rotation"}   = "ordered: zyx ".(12.0*$n)."*deg 0*deg ".(-$mirrAng3)."*deg";
	$detector{"color"}      = "66bbff";
	$detector{"type"}       = "Cons";
	$detector{"dimensions"} = sprintf('%.3f',$R_front_in)."*cm ". sprintf('%.3f',$R_front_out)."*cm ".sprintf('%.3f',$R_end_in)."*cm ".sprintf('%.3f',$R_end_out)."*cm ".sprintf('%.3f',0.5*($Z_end - $Z_front))."*cm "." 84*deg 12*deg";
	$detector{"material"}   = "Component";
	$detector{"mfield"}     = "no";
	$detector{"ncopy"}      = 1;
	$detector{"pMany"}       = 1;
	$detector{"exist"}       = 1;
	$detector{"visible"}     = 1;
	$detector{"style"}       = 1;
	$detector{"sensitivity"} = "no";
	$detector{"hit_type"}    = "";
	$detector{"identifiers"} = "Mirror3 cone $n";
	
	print_det(\%configuration, \%detector);

	$PosV_temp_CM = &rotateZv($PosV_temp_CM,12.0*$D2R);	
	
	$detector{"name"}        = $namePre."Mirror3_$n";
	$detector{"mother"}      = $namePre."Tank" ;
	$detector{"description"} = "Mirror3 segment $n";
	$detector{"pos"}        = sprintf('%.3f',$PosV_temp_CM->x())."*cm ".sprintf('%.3f',$PosV_temp_CM->y())."*cm ".sprintf('%.3f',$PosV_temp_CM->z())."*cm";
	$detector{"rotation"}   = "0*deg 0*deg 0*deg";
	$detector{"color"}      = "111166";
	$detector{"type"}       = "Operation:@ ".$namePre."Mirror3s_$n * ".$namePre."Mirror3c_$n";
	$detector{"material"}   = "PVDIS_mirror";
	$detector{"mfield"}     = "no";
	$detector{"ncopy"}      = 1;
	$detector{"pMany"}       = 1;
	$detector{"exist"}       = 1;
	$detector{"visible"}     = 1;
	$detector{"style"}       = 1;
	$detector{"sensitivity"} = "Mirrors";
	$detector{"hit_type"}    = "Mirrors";
	$detector{"identifiers"} = "Mirror WithSurface: 0 With id: ".($n+230)." WithBorderVolume: MirrorSkin 0 WithMaterial: PVDIS_mirror";

	print_det(\%configuration, \%detector);


    }
}1;
