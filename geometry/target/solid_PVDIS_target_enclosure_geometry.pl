...
sub solid_PVDIS_target_enclosure
{
# the first argument to this function becomes the variation
    $configuration{"variation"} = shift;
    
my $target;    
if ($configuration{"variation"} =~ /LD2/){
  $target="LD2";
}
elsif ($configuration{"variation"} =~ /LH2/){
  $target="G4_lH2";
}
else {
  printf "not sure about target material";
}
    
 my $NUM  = 6;
 my @z    = (0.+$targetoff,0.,-36.99,36.99,0.,0.);
 my @Rin  = (0.,0.,0.,0.,0.,0.);
 my @Rout = (45,44.95,30.,5.,2.54,2.5);
 ...
 my @mat  = ("G4_Al","G4_Galactic","G4_Galactic","G4_Galactic","G4_Al","$target");
 my @color = ("0000ff","808080","808080","808080","0000ff","ff0000");
