...
TH1F *hnphe_hgc=new TH1F("hnphe_hgc","hnphe_hgc",1000,0,10);
TH1F *hnphe_lgc=new TH1F("hnphe_lgc","hnphe_lgc",100,-0.5,99.5);
...
//       if (detector_ID==3 && subdetector_ID == 1 && subsubdetector_ID == 1)   cout << "particle mom entering EC " << flux_trackE->at(j) << endl;         
     ...
     double p_hit=sqrt(flux_px->at(j)*flux_px->at(j)+flux_py->at(j)*flux_py->at(j)+flux_pz->at(j)*flux_pz->at(j));
...
//   cout << "npe_hgc " << npe_hgc << endl;  
 if (npe_hgc >0)  hnphe_hgc->Fill(npe_hgc/30./5e8*(30e-9*15e-6/1.6e-19));  
...
//     if (nphe_lgc_total>20){  // cut on lgc
      Is_acc_FA=true;
//     }
    }       
  }
