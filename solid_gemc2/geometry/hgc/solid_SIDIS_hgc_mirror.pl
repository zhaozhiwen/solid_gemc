...
{    
	...
	%mat= init_mir();
	$mat{"name"}         = "SL_HGC_mirror";
	$mat{"description"}  = "SL_HGC_mirror";
	...
	%mat = init_mir();
	$mat{"name"}         = "SL_HGC_mirror_pmt";
	$mat{"description"}  = "SL_HGC_mirror_pmt";
	$mat{"type"}         = "dielectric_metal";
	$mat{"finish"}       = "polished";
	$mat{"model"}        = "unified";
	$mat{"border"}       = "SkinSurface";
	...
	$mat{"efficiency"}   = arrayToString(@efficiency_pmt);	
	print_mir(\%configuration, \%mat);	
