...
{
make_solid_JPsi_virtualplane_muon_forwardangle();
...
sub make_solid_JPsi_virtualplane_muon_forwardangle
{
 my %detector=init_det();
 $detector{"name"}        = "$DetectorName\_forwardangle";
...
sub make_solid_JPsi_virtualplane_muon_endcapdonut
{
 ...
 $detector{"sensitivity"} = "flux";
 $detector{"hit_type"}    = "flux";
 $detector{"identifiers"} = "id manual 6200000";
 print_det(\%configuration, \%detector);
}
...
sub make_solid_JPsi_virtualplane_muon_barrel_1
{
 ...
 $detector{"sensitivity"} = "flux";
 $detector{"hit_type"}    = "flux";
 $detector{"identifiers"} = "id manual 6300000";
 print_det(\%configuration, \%detector);
}
...
sub make_solid_JPsi_virtualplane_muon_barrel_2
{
 ...
 $detector{"sensitivity"} = "flux";
 $detector{"hit_type"}    = "flux";
 $detector{"identifiers"} = "id manual 6400000";
 print_det(\%configuration, \%detector);
}
